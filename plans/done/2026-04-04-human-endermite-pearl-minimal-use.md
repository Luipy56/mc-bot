# Track endermite spawn risk; avoid pearl spam in stronghold corridors

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_endermite_pearl_minimal_use`.
...
2. Register `taskId` `human_human_endermite_pearl_minimal_use` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-endermite-pearl-minimal-use.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
