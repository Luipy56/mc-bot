# When big slime splits, target small cubes nearest to weapon reach first to reduce chip damage

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_slime_small_swarm_priority`.
...
2. Register `taskId` `human_human_slime_small_swarm_priority` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-slime-small-swarm-priority.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
