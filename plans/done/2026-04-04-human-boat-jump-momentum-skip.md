# Use boat on ice edge jump tricks only when HP high and gap measured (opt-in risky)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_boat_jump_momentum_skip`.
...
2. Register `taskId` `human_human_boat_jump_momentum_skip` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-boat-jump-momentum-skip.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
