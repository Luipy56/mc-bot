# Keep one dedicated food stack in a predictable slot and avoid merging it into bulk processing by accident

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_reserve_emergency_food_slot`.
...
2. Register `taskId` `human_human_reserve_emergency_food_slot` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-reserve-emergency-food-slot.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
