# In water, favor surfacing cadence and avoid digging straight up into gravel while oxygen is low

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_swim_surface_breathe_gap`.
...
2. Register `taskId` `human_human_swim_surface_breathe_gap` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-swim-surface-breathe-gap.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
