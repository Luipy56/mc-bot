# Disable sprint near steep drops or when adjacent air below feet; prefer walk speed until terrain is safe

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_walk_cliff_no_sprint`.
...
2. Register `taskId` `human_human_walk_cliff_no_sprint` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-walk-cliff-no-sprint.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
