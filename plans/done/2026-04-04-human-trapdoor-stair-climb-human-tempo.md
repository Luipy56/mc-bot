# Climb trapdoor staircases with deliberate delay between opens to avoid head bonk loops

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_trapdoor_stair_climb_human_tempo`.
...
2. Register `taskId` `human_human_trapdoor_stair_climb_human_tempo` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-trapdoor-stair-climb-human-tempo.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
