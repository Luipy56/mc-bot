# Skip pickup of low-value hunger items when inventory pressure is high unless truly starving

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_ignore_rotten_flesh_nearly_full`.
...
2. Register `taskId` `human_human_ignore_rotten_flesh_nearly_full` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-ignore-rotten-flesh-nearly-full.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
