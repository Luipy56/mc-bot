# Eat melon slices instead of breaking full blocks for food when slices exist

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_melon_consume_slices_only`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (survival;farming), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_melon_consume_slices_only` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-melon-consume-slices-only.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
