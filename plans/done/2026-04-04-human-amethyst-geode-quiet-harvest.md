# Harvest buds with silk expectation check; avoid loud unnecessary block breaks inside geode

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_amethyst_geode_quiet_harvest`.
...
2. Register `taskId` `human_human_amethyst_geode_quiet_harvest` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-amethyst-geode-quiet-harvest.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
