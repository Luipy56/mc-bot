# Open bastion chest only after gold distraction drop and escape route marked

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bastion_chest_piglin_milk`.
...
2. Register `taskId` `human_human_bastion_chest_piglin_milk` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bastion-chest-piglin-milk.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
