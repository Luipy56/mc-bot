# Reorder hotbar to a stable human-like layout: tools left, blocks middle, food right when idle at chest

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_hotbar_order_tool_blocks_food`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (inventory), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_hotbar_order_tool_blocks_food` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-hotbar-order-tool-blocks-food.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
