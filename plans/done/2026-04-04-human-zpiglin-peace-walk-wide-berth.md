# Path around zombified piglin packs with margin; never sprint-punch through neutral crowds

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_zpiglin_peace_walk_wide_berth`.
...
2. Register `taskId` `human_human_zpiglin_peace_walk_wide_berth` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-zpiglin-peace-walk-wide-berth.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
