# Use recovery compass target to bias explore return path after death respawn

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_recovery_compass_death_run`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (survival;utility), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_recovery_compass_death_run` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-recovery-compass-death-run.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
