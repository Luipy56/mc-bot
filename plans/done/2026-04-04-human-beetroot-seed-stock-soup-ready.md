# Keep a minimum beetroot seed buffer before converting all to soup components

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_beetroot_seed_stock_soup_ready`.
...
2. Register `taskId` `human_human_beetroot_seed_stock_soup_ready` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-beetroot-seed-stock-soup-ready.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
