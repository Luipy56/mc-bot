# Open fence gates only as wide as needed, re-close after passage to reduce mob leakage

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_fence_gate_click_through_mule`.
...
2. Register `taskId` `human_human_fence_gate_click_through_mule` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-fence-gate-click-through-mule.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
