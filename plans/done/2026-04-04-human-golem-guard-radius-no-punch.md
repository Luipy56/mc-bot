# Never accidentally punch iron golem while clicking doors or chests near village center

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_golem_guard_radius_no_punch`.
...
2. Register `taskId` `human_human_golem_guard_radius_no_punch` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-golem-guard-radius-no-punch.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
