# Before anvil merges, grindstone trash enchants off cheap gear to recover small XP

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_grindstone_disenchant_junk_first`.
...
2. Register `taskId` `human_human_grindstone_disenchant_junk_first` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-grindstone-disenchant-junk-first.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
