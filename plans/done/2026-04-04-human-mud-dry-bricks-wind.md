# Pack mud into clay renew mindset: dry mud bricks for early decorative walls near rivers

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_mud_dry_bricks_wind`.
...
2. Register `taskId` `human_human_mud_dry_bricks_wind` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-mud-dry-bricks-wind.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
