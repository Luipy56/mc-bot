# Against a single melee mob, interleave short retreats and weapon swings instead of standing still

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_backpedal_melee_kite`.

## Summary

Against a single melee mob, interleave short retreats and weapon swings instead of standing still. Improves survivability on low armor.
...
2. Register `taskId` `human_human_backpedal_melee_kite` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-backpedal-melee-kite.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
