# Listen for goat ram audio; sidestep near powder snow edges

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_powder_snow_goat_horn_dodge`.
...
2. Register `taskId` `human_human_powder_snow_goat_horn_dodge` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-powder-snow-goat-horn-dodge.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
