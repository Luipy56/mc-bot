# When standing idle safe, apply tiny random yaw updates within bounds (not full spin)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_idle_camera_micro_yaw_noise`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (humanize;movement), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_idle_camera_micro_yaw_noise` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-idle-camera-micro-yaw-noise.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
