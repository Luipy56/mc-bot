# When thunder starts, pause outdoor tower building and seek roof or shallow cave mouth

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_thunder_hide_under_roof`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (survival;weather), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_thunder_hide_under_roof` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-thunder-hide-under-roof.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
