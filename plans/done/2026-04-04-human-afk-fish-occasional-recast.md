# While fishing, add occasional recast jitter and small look drift to avoid perfect bot rhythm

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_afk_fish_occasional_recast`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (gathering;humanize), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_afk_fish_occasional_recast` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-afk-fish-occasional-recast.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
