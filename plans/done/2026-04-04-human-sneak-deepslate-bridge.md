# Cross narrow deepslate lips over void or lava on sneak with slower pathfinder max speed

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_sneak_deepslate_bridge`.
...
2. Register `taskId` `human_human_sneak_deepslate_bridge` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-sneak-deepslate-bridge.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
