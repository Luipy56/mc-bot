# On darkness effect, place temporary light or retreat along memorized path

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_warden_darkness_panic_torch`.
...
2. Register `taskId` `human_human_warden_darkness_panic_torch` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-warden-darkness-panic-torch.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
