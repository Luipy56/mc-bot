# Place lamps triggered by daylight sensor for automatic “home lights at dusk” feel

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_daylight_lamp_skylight_base`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (redstone;building), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_daylight_lamp_skylight_base` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-daylight-lamp-skylight-base.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
