# Throw lingering harm outward while retreating from swarm mobs (best-effort)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_lingering_cloud_kite`.
...
2. Register `taskId` `human_human_lingering_cloud_kite` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-lingering-cloud-kite.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
