# When spider is above, prioritize upward aim and jump timing instead of ground sweeps

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_spider_ceiling_look_up_strike`.
...
2. Register `taskId` `human_human_spider_ceiling_look_up_strike` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-spider-ceiling-look-up-strike.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
