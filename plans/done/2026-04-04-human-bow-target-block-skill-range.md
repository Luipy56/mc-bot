# Practice bow on target block at safe range to calibrate hold time before real fights

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bow_target_block_skill_range`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (combat;utility), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_bow_target_block_skill_range` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bow-target-block-skill-range.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
