# Periodically quick-look behind when walking away from combat zones or after hearing hurt sounds

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_180_glance_hostile_rear`.
...
2. Register `taskId` `human_human_180_glance_hostile_rear` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-180-glance-hostile-rear.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
