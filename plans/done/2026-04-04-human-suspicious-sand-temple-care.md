# Brush desert temple suspicious sand from side supports to avoid plate detonation

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_suspicious_sand_temple_care`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (gathering;safety), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_suspicious_sand_temple_care` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-suspicious-sand-temple-care.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
