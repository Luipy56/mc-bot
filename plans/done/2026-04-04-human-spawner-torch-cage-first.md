# Light all faces of a found spawner room before breaking cage blocks

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_spawner_torch_cage_first`.
...
2. Register `taskId` `human_human_spawner_torch_cage_first` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-spawner-torch-cage-first.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
