# If both exist, prefer cooked stacks for saturation; raw only when desperate or for breeding/trading prep

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_prefer_cooked_meat_raw`.
...
2. Register `taskId` `human_human_prefer_cooked_meat_raw` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-prefer-cooked-meat-raw.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
