# Use lever to release minecart at station stops instead of punching cart off rails

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_lever_cart_station_hop`.
...
2. Register `taskId` `human_human_lever_cart_station_hop` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-lever-cart-station-hop.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
