# Ring village bell during raid entry to gather villagers indoors (interaction hook)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bell_ring_panic_clear`.
...
2. Register `taskId` `human_human_bell_ring_panic_clear` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bell-ring-panic-clear.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
