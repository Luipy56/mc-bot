# Name or slot shulker boxes by color for kit categories (mining, potions, building)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_shulker_color_code_kits`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (inventory;end), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_shulker_color_code_kits` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-shulker-color-code-kits.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
