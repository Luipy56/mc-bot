# During crystal phase, break line of sight to dragon breath between shots

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_crystal_tower_duck_behind_pillar`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (end;combat), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_crystal_tower_duck_behind_pillar` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-crystal-tower-duck-behind-pillar.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
