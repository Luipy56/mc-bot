# Surround enchanting table with bookshelf count that matches desired power tier before rolling

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bookshelf_enchant_power_ring`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (gear), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_bookshelf_enchant_power_ring` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bookshelf-enchant-power-ring.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
