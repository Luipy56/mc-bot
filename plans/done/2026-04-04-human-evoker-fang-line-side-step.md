# Strafe perpendicular to evoker fang line pattern on first visual cue

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_evoker_fang_line_side_step`.
...
2. Register `taskId` `human_human_evoker_fang_line_side_step` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-evoker-fang-line-side-step.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
