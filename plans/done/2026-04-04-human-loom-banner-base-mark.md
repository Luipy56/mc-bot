# Craft simple banner pattern as team/base marker on shields later in chain

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_loom_banner_base_mark`.
...
2. Register `taskId` `human_human_loom_banner_base_mark` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-loom-banner-base-mark.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
