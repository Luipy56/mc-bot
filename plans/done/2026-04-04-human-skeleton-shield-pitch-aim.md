# Adjust look pitch toward skeleton head height when blocking arrows with shield

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_skeleton_shield_pitch_aim`.
...
2. Register `taskId` `human_human_skeleton_shield_pitch_aim` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-skeleton-shield-pitch-aim.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
