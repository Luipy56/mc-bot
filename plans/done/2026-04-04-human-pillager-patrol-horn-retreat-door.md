# On raid horn distant, preemptively move toward defensible door and close gates

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_pillager_patrol_horn_retreat_door`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (combat;social), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_pillager_patrol_horn_retreat_door` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-pillager-patrol-horn-retreat-door.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
