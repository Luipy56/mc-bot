# Place campfire smoke near hive before bottle/shear operations to reduce anger odds

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bee_angry_smoke_campfire_calm`.
...
2. Register `taskId` `human_human_bee_angry_smoke_campfire_calm` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bee-angry-smoke-campfire-calm.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
