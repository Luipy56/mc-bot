# Block movement input during consume animation so food registers reliably like a standing player

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_stand_still_while_eating`.
...
2. Register `taskId` `human_human_stand_still_while_eating` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-stand-still-while-eating.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
