# Charge respawn anchor safely in enclosed room with explosion padding calculation

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_anchor_charge_respawn_nether`.
...
2. Register `taskId` `human_human_anchor_charge_respawn_nether` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-anchor-charge-respawn-nether.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
