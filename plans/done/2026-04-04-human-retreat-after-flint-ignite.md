# After lighting portal or netherrack, step back a few ticks before hostiles or fire spread react

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_retreat_after_flint_ignite`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (nether;safety), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_retreat_after_flint_ignite` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-retreat-after-flint-ignite.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
