# Write short signs or item-frame codes above grouped chests for base orientation (bot-readable blackboard sync)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_sign_chest_row_labels`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (building;utility), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_sign_chest_row_labels` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-sign-chest-row-labels.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
