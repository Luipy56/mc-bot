# When crossing narrow lava, place basalt/cobble stepping with sneak between placements

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_lava_sea_basalt_stepping_stones`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (mining;nether), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_lava_sea_basalt_stepping_stones` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-lava-sea-basalt-stepping-stones.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
