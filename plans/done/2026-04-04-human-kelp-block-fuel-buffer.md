# Compact dried kelp blocks as furnace fuel buffer before long smelt sessions

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_kelp_block_fuel_buffer`.
...
2. Register `taskId` `human_human_kelp_block_fuel_buffer` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-kelp-block-fuel-buffer.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
