# Wax copper builds selected for aesthetics to prevent ugly patchy oxidation on bases

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_copper_oxidize_wax_preserve`.
...
2. Register `taskId` `human_human_copper_oxidize_wax_preserve` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-copper-oxidize-wax-preserve.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
