# Throttle milk attempts per cow with spacing so animations complete and server accepts

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_cow_milk_cooldown_throttle_space`.
...
2. Register `taskId` `human_human_cow_milk_cooldown_throttle_space` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-cow-milk-cooldown-throttle-space.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
