# Never feed cookies to parrots; use seeds only (hard safety invariant)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_parrot_no_chocolate_ever`.
...
2. Register `taskId` `human_human_parrot_no_chocolate_ever` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-parrot-no-chocolate-ever.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
