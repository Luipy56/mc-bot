# If breaking stone spawns silverfish in stronghold, switch to sword and stop pickaxe tunnel

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_silverfish_stop_on_infest_hint`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (mining;combat), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_silverfish_stop_on_infest_hint` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-silverfish-stop-on-infest-hint.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
