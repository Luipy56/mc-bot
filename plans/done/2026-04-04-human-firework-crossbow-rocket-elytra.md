# Support rocket-boost via crossbow-held fireworks when main hand busy (version gated)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_firework_crossbow_rocket_elytra`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (movement;end), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_firework_crossbow_rocket_elytra` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-firework-crossbow-rocket-elytra.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
