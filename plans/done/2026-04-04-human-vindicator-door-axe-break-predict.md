# If vindicator is chopping door, either swap to blocking or reposition before door breaks

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_vindicator_door_axe_break_predict`.
...
2. Register `taskId` `human_human_vindicator_door_axe_break_predict` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-vindicator-door-axe-break-predict.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
