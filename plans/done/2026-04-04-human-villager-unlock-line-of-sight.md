# Step so villager can see workstation for profession refresh before trading spam

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_villager_unlock_line_of_sight`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (social), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_villager_unlock_line_of_sight` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-villager-unlock-line-of-sight.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
