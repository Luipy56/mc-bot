# When fireball close, prioritize punch reflect angle over panic sprint into lava

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_ghast_deflect_angry_return`.
...
2. Register `taskId` `human_human_ghast_deflect_angry_return` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-ghast-deflect-angry-return.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
