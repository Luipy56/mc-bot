# Hide low-value item frame on “empty” wall panel that marks stash behind (metadata only)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_item_frame_secret_stash_cover`.
...
2. Register `taskId` `human_human_item_frame_secret_stash_cover` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-item-frame-secret-stash-cover.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
