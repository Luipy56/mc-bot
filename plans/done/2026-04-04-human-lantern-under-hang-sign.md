# Place lanterns under hanging signs or chains for ceiling light without full block loss of headroom

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_lantern_under_hang_sign`.
...
2. Register `taskId` `human_human_lantern_under_hang_sign` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-lantern-under-hang-sign.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
