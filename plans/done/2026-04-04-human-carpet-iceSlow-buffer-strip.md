# Lay short carpet strips before packed ice paths to shed speed before cliffs (human panic brake)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_carpet_iceSlow_buffer_strip`.
...
2. Register `taskId` `human_human_carpet_iceSlow_buffer_strip` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-carpet-iceSlow-buffer-strip.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
