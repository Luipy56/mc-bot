# Wire redstone lamp floor segment toggled by pressure plate entrance for foyer

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_redstone_lamp_floor_night`.
...
2. Register `taskId` `human_human_redstone_lamp_floor_night` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-redstone-lamp-floor-night.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
