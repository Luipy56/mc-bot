# Never assume template duplication; plan one-time trim template usage carefully

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_smithing_template_duping_skip`.
...
2. Register `taskId` `human_human_smithing_template_duping_skip` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-smithing-template-duping-skip.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
