# Raise shield for shulker bullets when cover blocks unavailable; strafe toward pillar

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_shulker_cover_shield_bullets`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (combat;end), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_shulker_cover_shield_bullets` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-shulker-cover-shield-bullets.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
