# Brush suspicious gravel near fossils for bone blocks without destroying whole vein

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_fossil_bone_meal_brush`.
...
2. Register `taskId` `human_human_fossil_bone_meal_brush` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-fossil-bone-meal-brush.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
