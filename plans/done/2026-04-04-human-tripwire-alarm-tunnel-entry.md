# String tripwire across branch mine entrance for early hostile alert (hear click)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_tripwire_alarm_tunnel_entry`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (redstone;mining), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_tripwire_alarm_tunnel_entry` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-tripwire-alarm-tunnel-entry.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
