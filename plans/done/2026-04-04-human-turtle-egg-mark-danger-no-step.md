# Mark turtle egg zones in blackboard and pathfind around sand patches in bases

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_turtle_egg_mark_danger_no_step`.
...
2. Register `taskId` `human_human_turtle_egg_mark_danger_no_step` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-turtle-egg-mark-danger-no-step.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
