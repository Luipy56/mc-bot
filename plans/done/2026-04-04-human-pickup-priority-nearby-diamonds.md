# When multiple drops compete, bias pickup toward ores and combat drops over stone/cobble

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_pickup_priority_nearby_diamonds`.
...
2. Register `taskId` `human_human_pickup_priority_nearby_diamonds` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-pickup-priority-nearby-diamonds.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
