# Before mining up through gravel, place torch pillar break trick or side-step column

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_gravel_ceiling_safe_tap`.
...
2. Register `taskId` `human_human_gravel_ceiling_safe_tap` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-gravel-ceiling-safe-tap.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
