# Use small bamboo raft craft and single crossing when boat unavailable but bamboo plentiful

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bamboo_raft_river_cross`.
...
2. Register `taskId` `human_human_bamboo_raft_river_cross` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bamboo-raft-river-cross.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
