# After poison applied, path to milk bucket use with safe timeout (clear effects)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_witch_milk_after_poison`.
...
2. Register `taskId` `human_human_witch_milk_after_poison` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-witch-milk-after-poison.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
