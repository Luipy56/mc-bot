# For openings, prefer glass panes over full blocks when building “windows” for light with less material

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_glass_pane_slim_windows`.
...
2. Register `taskId` `human_human_glass_pane_slim_windows` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-glass-pane-slim-windows.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
