# When ravager roar stun expected, pre-raise shield and angle toward impact side

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_ravager_stun_shield_react`.
...
2. Register `taskId` `human_human_ravager_stun_shield_react` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-ravager-stun-shield-react.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
