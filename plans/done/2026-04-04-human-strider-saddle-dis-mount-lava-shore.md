# Dismount strider only on safe shore blocks; never jump off over open lava

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_strider_saddle_dis_mount_lava_shore`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (nether;movement), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_strider_saddle_dis_mount_lava_shore` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-strider-saddle-dis-mount-lava-shore.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
