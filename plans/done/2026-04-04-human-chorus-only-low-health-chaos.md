# Consume chorus fruit teleport only when trapped or critically low, not for casual travel

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_chorus_only_low_health_chaos`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (end;survival), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_chorus_only_low_health_chaos` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-chorus-only-low-health-chaos.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
