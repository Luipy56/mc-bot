# Keep lateral distance from drowned with visible trident; use shield or terrain breaks

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_drowned_ranged_trident_kite`.
...
2. Register `taskId` `human_human_drowned_ranged_trident_kite` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-drowned-ranged-trident-kite.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
