# Time shield for hoglin charge knockback windows; avoid trading during stun

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_hoglin_shield_timed_block`.
...
2. Register `taskId` `human_human_hoglin_shield_timed_block` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-hoglin-shield-timed-block.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
