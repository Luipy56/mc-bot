# Place lightning rod atop highest base column to protect wooden roofs during storms

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_lightning_rod_chimney_stack`.
...
2. Register `taskId` `human_human_lightning_rod_chimney_stack` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-lightning-rod-chimney-stack.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
