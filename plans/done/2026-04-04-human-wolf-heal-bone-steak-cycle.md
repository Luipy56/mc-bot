# Heal tamed wolves with meat when low; sit/stand commands respected around cliffs

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_wolf_heal_bone_steak_cycle`.
...
2. Register `taskId` `human_human_wolf_heal_bone_steak_cycle` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-wolf-heal-bone-steak-cycle.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
