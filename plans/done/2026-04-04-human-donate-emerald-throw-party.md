# Rare idle: toss single emerald near villager as flavor “gift” if surplus high (server rules permitting)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_donate_emerald_throw_party`.
...
2. Register `taskId` `human_human_donate_emerald_throw_party` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-donate-emerald-throw-party.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
