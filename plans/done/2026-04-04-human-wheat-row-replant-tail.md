# After harvesting a wheat row, replant from the far end backward so the bot does not trample fresh soil

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_wheat_row_replant_tail`.
...
2. Register `taskId` `human_human_wheat_row_replant_tail` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-wheat-row-replant-tail.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
