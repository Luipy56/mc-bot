# Smelt quartz or cactus for small XP grind before enchant rerolls (human prep)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_grindstone_xp_bank_smelt`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (processing;gear), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_grindstone_xp_bank_smelt` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-grindstone-xp-bank-smelt.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
