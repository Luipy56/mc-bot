# Use mud bricks as trim layers on wooden houses for contrast like player builds

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_mudbrick_wall_trim`.
...
2. Register `taskId` `human_human_mudbrick_wall_trim` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-mudbrick-wall-trim.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
