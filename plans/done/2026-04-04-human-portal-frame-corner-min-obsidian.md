# Build portal frames using corner-first placement habit to reduce miscount errors

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_portal_frame_corner_min_obsidian`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (nether), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_portal_frame_corner_min_obsidian` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-portal-frame-corner-min-obsidian.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
