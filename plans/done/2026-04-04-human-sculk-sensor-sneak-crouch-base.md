# Move on sneak past sculk sensors in ancient city when warden risk mode enabled

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_sculk_sensor_sneak_crouch_base`.
...
2. Register `taskId` `human_human_sculk_sensor_sneak_crouch_base` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-sculk-sensor-sneak-crouch-base.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
