# If another player is within chat range and non-hostile, occasional arm swing “wave” (no chat spam)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_near_player_arm_swing_greeting`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (social;humanize), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_near_player_arm_swing_greeting` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-near-player-arm-swing-greeting.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
