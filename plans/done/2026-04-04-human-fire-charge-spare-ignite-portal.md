# If flint steel durability low, attempt fire charge ignition when available as backup

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_fire_charge_spare_ignite_portal`.
...
2. Register `taskId` `human_human_fire_charge_spare_ignite_portal` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-fire-charge-spare-ignite-portal.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
