# Avoid picking wither rose without hoe/space plan; mark dangerous floor patches

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_wither_rose_pick_soulsand_skip`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (nether;survival), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_wither_rose_pick_soulsand_skip` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-wither-rose-pick-soulsand-skip.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
