# After pathing through a door to escape mobs, interact to close it if rules allow and no ally follows

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_close_door_after_flee_indoors`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (building;survival), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_close_door_after_flee_indoors` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-close-door-after-flee-indoors.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
