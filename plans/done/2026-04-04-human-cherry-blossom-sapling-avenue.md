# Plant cherry saplings in avenue pattern near base for landmark navigation memory

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_cherry_blossom_sapling_avenue`.
...
2. Register `taskId` `human_human_cherry_blossom_sapling_avenue` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-cherry-blossom-sapling-avenue.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
