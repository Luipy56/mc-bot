# Prefer silk touch night harvest with campfire calm and clear escape path

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_beehive_silk_night_harvest_safe`.
...
2. Register `taskId` `human_human_beehive_silk_night_harvest_safe` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-beehive-silk-night-harvest-safe.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
