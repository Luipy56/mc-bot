# Use doorway peeks to limit blaze line of sight instead of open fortress hall standoff

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_blaze_doorframe_snipe`.
...
2. Register `taskId` `human_human_blaze_doorframe_snipe` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-blaze-doorframe-snipe.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
