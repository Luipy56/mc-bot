# Consistent torch placement on left wall in branch mines for human “breadcrumb” symmetry

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_branch_mine_torch_left_wall`.
...
2. Register `taskId` `human_human_branch_mine_torch_left_wall` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-branch-mine-torch-left-wall.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
