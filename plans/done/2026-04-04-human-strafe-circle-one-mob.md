# When locked onto one hostile in open terrain, orbit at weapon range with small radius changes

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_strafe_circle_one_mob`.
...
2. Register `taskId` `human_human_strafe_circle_one_mob` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-strafe-circle-one-mob.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
