# Pack food and kelp snacks into bundle for long cave trips when bundle item exists

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_bundle_sort_food_kelp`.
...
2. Register `taskId` `human_human_bundle_sort_food_kelp` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-bundle-sort-food-kelp.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
