# When dragon perches, aim with slight lead based on animation phase (best-effort)

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_dragon_perch_bow_lead_target`.
...
2. Register `taskId` `human_human_dragon_perch_bow_lead_target` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-dragon-perch-bow-lead-target.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
