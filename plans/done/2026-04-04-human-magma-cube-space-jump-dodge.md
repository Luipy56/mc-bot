# Use vertical spacing and backward hops against large magma cubes in basalt deltas

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_magma_cube_space_jump_dodge`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (combat;nether), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_magma_cube_space_jump_dodge` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-magma-cube-space-jump-dodge.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
