# When entering risky PvE clusters, pre-equip totem offhand before first hit arrives

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_totem_offhand_swap_predict`.
...
2. Register `taskId` `human_human_totem_offhand_swap_predict` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-totem-offhand-swap-predict.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
