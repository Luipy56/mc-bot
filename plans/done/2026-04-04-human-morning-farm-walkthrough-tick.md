# At dawn, walk crop rows once checking maturity before mining tasks resume

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_morning_farm_walkthrough_tick`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (farming;routine), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_morning_farm_walkthrough_tick` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-morning-farm-walkthrough-tick.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
