# Route raw ores to blast furnace when block exists near base for throughput

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_blast_furnace_ore_priority`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (processing), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_blast_furnace_ore_priority` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-blast-furnace-ore-priority.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
