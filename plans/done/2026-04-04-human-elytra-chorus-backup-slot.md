# Keep chorus fruit in dedicated slot when elytra flying over void for emergency pop

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_elytra_chorus_backup_slot`.
...
**In scope:** New or extended behavior entirely under `autonomous/` (end;movement), wired through existing skill/executor patterns where possible; tests under `autonomous/tests/`; optional `playerSkillCatalog.js` row with status `live-addon` or `extended` until stable.
...
2. Register `taskId` `human_human_elytra_chorus_backup_slot` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-elytra-chorus-backup-slot.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
