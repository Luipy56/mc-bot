# Compact piston door with memorized block break order for exit during raids

**Status:** implemented
**Related:** Jarvys human-play backlog; `autonomous/`; proposed catalog `taskId` `human_human_piston_door_thumb_pattern`.
...
2. Register `taskId` `human_human_piston_door_thumb_pattern` in `autonomous/bot.js` if exposed as a task; add entry to `autonomous/lib/playerSkillCatalog.js`. **Done:** dispatch resolves without throw.
3. Add `autonomous/tests/human-piston-door-thumb-pattern.test.js` or extend the nearest behavioral test with mocks. **Done:** `cd autonomous && npm test` passes.
